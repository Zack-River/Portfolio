  ...
  return (
    ...
    >
      <div className="w-full max-w-7xl mx-auto px-6 md:px-12 lg:px-24 relative z-10">
        <GSAPReveal>
          <SectionHeader
            title={SITE_CONTENT.about.sectionHeader.title}
            subtitle={SITE_CONTENT.about.sectionHeader.subtitle}
            number={SITE_CONTENT.about.sectionHeader.number}
          />
        </GSAPReveal>
              ...
              <GSAPReveal delay={0.2}>
                <div className="pt-8 md:pt-16">
                  <h3 className="text-sm font-bold uppercase tracking-[0.2em] text-electric mb-8 flex items-center justify-center md:justify-start gap-3">
                    ...
                    {SITE_CONTENT.about.educationTitle}
                  </h3>

                  {/* Mobile */}
                  <div className="md:hidden py-2 flex flex-col items-center text-center w-full">
                    <h4 className="text-base text-canvas-light font-bold tracking-wide uppercase mb-1">
                      {EDUCATION.degree}
                    </h4>
                    <p className="text-canvas-light/60 text-xs mb-4 tracking-widest uppercase">
                      {EDUCATION.institution}
                    </p>
                    <div className="flex justify-center space-x-4 items-center text-[9px] font-mono text-canvas-light/50">
                      <span className="bg-white/5 px-3 py-1 rounded-sm border border-white/10 tracking-widest uppercase">
                        {EDUCATION.period}
                      </span>
                  ...
                  <div className="hidden md:block border-l border-white/10 pl-6 py-2 relative w-full">
                    ...
                    <h4 className="text-lg text-canvas-light font-bold tracking-wide uppercase mb-1">
                      {EDUCATION.degree}
                    </h4>
                    <p className="text-canvas-light/60 text-xs mb-4 tracking-widest uppercase">
                      {EDUCATION.institution}
            ...
            <div className="space-y-8">
              <GSAPReveal delay={0.3}>
                <div className="pt-8">
                  <h3 className="text-2xl font-display text-canvas-dark dark:text-canvas-light mb-6 flex items-center justify-center md:justify-start gap-3">
                    ...
                    {SITE_CONTENT.about.experienceTitle}
                  </h3>
                </div>
              </GSAPReveal>
              ...
              <div className="space-y-8">
                ...
                <div className="md:hidden relative w-full mb-4">
                  ...
                  >
                    {TRAINING.slice(0, -1).map((job, idx) => {
                      ...
                      return (
                        ...
                        >
                          <h4 className="text-xl text-primary dark:text-canvas-light font-semibold mb-1">
                            {job.role}
                          </h4>
                          <div className="flex flex-wrap items-center justify-center gap-2 mb-2">
                            <p className="text-xs font-mono text-electric bg-electric/10 px-3 py-1 rounded-full">
                              {job.period}
                            </p>
                            {job.certificate && (
                              <a
                          ...
                          <div
                            className={`overflow-hidden transition-all duration-500 ease-in-out w-full flex flex-col items-center ${isExpanded ? "max-h-125 opacity-100 mt-4" : "max-h-0 opacity-0 mt-0"}`}
                          >
                            ...
                            <ul className="space-y-3">
                              {job.details.map((detail, dIdx) => (
                                <li
                                  key={dIdx}
                                  className="text-secondary dark:text-canvas-light/70 text-sm leading-relaxed"
                                >
                                  {detail}
                                </li>
                              ))}
                          ...
                          <div className="mt-4 flex flex-col items-center justify-center text-canvas-dark/60 dark:text-white/60 hover:text-electric transition-colors">
                            ...
                            <span
                              className={`inline-block text-xs transition-transform duration-500 ${isExpanded ? "rotate-180" : ""}`}
                            >
                              ▼
                            </span>
                          </div>
                  ...
                  <button
                    onClick={() => scroll("left")}
                    className="absolute -left-4 top-1/2 -translate-y-1/2 z-10 p-2 bg-white/90 dark:bg-canvas-dark/80 backdrop-blur-sm rounded-full shadow-md text-electric border border-canvas-dark/10 dark:border-white/10 hover:bg-white dark:hover:bg-canvas-dark transition-colors"
                  ...
                  <button
                    ...
                    className="absolute -right-4 top-1/2 -translate-y-1/2 z-10 p-2 bg-white/90 dark:bg-canvas-dark/80 backdrop-blur-sm rounded-full shadow-md text-electric border border-canvas-dark/10 dark:border-white/10 hover:bg-white dark:hover:bg-canvas-dark transition-colors"
                ...
                <div
                  ref={timelineRef}
                  className="hidden md:block space-y-8 w-full relative"
                >
                  {TRAINING.slice(0, -1).map((job, idx) => (
                    <div
                      key={idx}
                      className="timeline-item border-l border-canvas-dark/10 dark:border-white/10 pl-6 py-2 relative group w-full"
                    >
                      ...
                      <div className="pl-4">
                        <div className="flex items-center gap-3 mb-1">
                          <h4 className="text-base font-display font-bold text-canvas-dark dark:text-canvas-light">
                            {job.role}
                          </h4>
                        </div>
                        <p className="text-secondary dark:text-canvas-light/60 text-sm mb-2">
                          {job.company}
                        </p>
                        <div className="flex flex-wrap items-center gap-3 mb-4">
                          <p className="text-xs font-mono text-secondary dark:text-canvas-light/50">
                            {job.period}
                          </p>
                          {job.certificate && (
                            <a
                        ...
                        <ul className="space-y-2 mb-4">
                          {job.details.map((detail, dIdx) => (
                            <li
                              key={dIdx}
                              className="text-secondary dark:text-canvas-light/70 text-sm pl-4 relative before:content-['-'] before:absolute before:left-0 before:text-muted"
                            >
                              {detail}
                            </li>
                          ))}
