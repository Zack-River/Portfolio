...
const About: React.FC = () => {
  return (
    ...
    >
      ...
      <div className="absolute -left-10 md:-left-20 top-1/2 -translate-y-1/2 text-[clamp(5rem,15vw,12rem)] font-bold text-canvas-dark/[0.03] font-display rotate-90 pointer-events-none origin-center">
        ABOUT
      </div>

      <div className="w-full max-w-7xl mx-auto px-4 md:px-12 lg:px-24 relative z-10">
        <Reveal>
          <SectionHeader
            title="The Developer"
          ...
          <div className="flex flex-col justify-between h-full space-y-8">
            <Reveal delay={0.2}>
              ...
              <div className="mb-8 relative group w-4/5 md:w-3/4 lg:w-2/3 mx-auto md:mx-0">
                <div className="absolute -inset-1 bg-gradient-to-r from-electric to-canvas-dark/80 rounded-2xl blur opacity-25 group-hover:opacity-75 transition duration-1000 group-hover:duration-200"></div>
                <img
                  src="/Zack.jpg"
              ...
              <p className="text-lg text-canvas-dark/80 font-light leading-relaxed text-center md:text-left w-4/5 mx-auto md:w-full md:mx-0 first-letter:text-5xl first-letter:font-display first-letter:mr-2 first-letter:float-left first-letter:text-electric">
                {PERSONAL_INFO.bio}
              </p>
              </div>
            </Reveal>
            ...
            <Reveal delay={0.4}>
              <div className="pt-8">
                ...
                <div className="border-l border-canvas-dark/10 pl-6 py-2 relative w-fit mx-auto md:mx-0 md:w-full">
                  ...
                  <h4 className="text-xl text-canvas-dark/80">
                    {EDUCATION.degree}
                  </h4>
                  <p className="text-canvas-dark/60 text-sm mb-2">
                    {EDUCATION.institution}
                  </p>
                  <div className="flex justify-start space-x-8 items-center text-xs font-mono text-canvas-dark/80">
              ...
              {TRAINING.map((job, idx) => (
                <Reveal key={idx} delay={0.5 + idx * 0.1}>
                  <div className="border-l border-canvas-dark/10 pl-6 py-2 relative group w-fit mx-auto md:mx-0 md:w-full">
                    ...
                    <h4 className="text-xl text-canvas-dark/80">{job.role}</h4>
                    <p className="text-canvas-dark/60 text-sm mb-2">
                      {job.company}
                    </p>
                    <p className="text-xs font-mono text-canvas-dark/80 mb-4">
                      {job.period}
                    </p>
                    <ul className="space-y-2">
                      {job.details.map((detail, dIdx) => (
                        <li
                          key={dIdx}
                          className="text-canvas-dark/80 text-sm pl-4 relative before:content-['-'] before:absolute before:left-0 before:text-canvas-dark/60"
                        >
                          {detail}
                        </li>
                      ))}
