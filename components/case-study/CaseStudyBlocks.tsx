...
import { ArrowLeft, ArrowRight, Download, ImageIcon, Maximize2 } from "lucide-react";
...
export const CaseStudyCta: React.FC<{ content: CaseStudyContent }> = ({ content }) => <section className="py-16 md:py-24 flex flex-col md:flex-row items-center justify-between gap-8"><div><p className="font-mono text-electric text-xs tracking-[0.2em] uppercase mb-3" dir={content.project.direction}>{content.cta.eyebrow}</p><h2 className="font-display text-3xl md:text-4xl font-bold">{content.cta.title}</h2></div><div className="flex flex-wrap items-center justify-center gap-3" dir={content.project.direction}>{content.cta.downloadHref && content.cta.downloadLabel && <a href={content.cta.downloadHref} target="_blank" rel="noopener noreferrer" className="btn-secondary inline-flex items-center justify-center gap-2" aria-label={content.cta.downloadLabel}><Download size={18} /> {content.cta.downloadLabel}</a>}<Link to={content.cta.href} className="btn-primary" aria-label={content.cta.label}>{content.cta.label} {content.project.direction === "rtl" ? <ArrowLeft size={18} /> : <ArrowRight size={18} />}</Link></div></section>;
