...
const Loader: React.FC<LoaderProps> = ({ onComplete }) => {
  ...
  useEffect(() => {
    ...
    const updateProgress = () => {
      ...
      progressTween = gsap.to(progressProxy, {
        ...
        onComplete: () => {
          if ((window as any)._loaderCheckReady) (window as any)._loaderCheckReady();
        }
      });
    };
  ...
  return (
    ...
    >
      ...
      {/* Central Content */}
      <div ref={logoRef} className="relative z-10 flex flex-col items-center justify-center gap-16 md:gap-20">
        
        {/* Logo & 3 Concentric Rings */}
        <div className="relative flex items-center justify-center">
          
          {/* Pulsing Background Glow */}
          <div ref={pulseBgRef} className="absolute w-45 h-45 md:w-62.5 md:h-62.5 rounded-full bg-electric/15 blur-[50px] opacity-20 z-0" />
          
          {/* Outer Ring: Slow faint arc */}
          <div className="absolute w-65 h-65 md:w-90 md:h-90 rounded-full border border-electric/10 border-t-electric/40 border-l-electric/10 animate-spin" style={{ animationDuration: '6s' }} />

          {/* Middle Ring: Dashed reverse rotating */}
          <div className="absolute w-52.5 h-52.5 md:w-72.5 md:h-72.5 rounded-full border border-dashed border-electric/30 opacity-60" style={{ animation: 'spin-reverse 10s linear infinite' }} />

          {/* Inner Ring: Solid fast glowing ring */}
          <div className="absolute w-40 h-40 md:w-55 md:h-55 rounded-full border-2 border-electric/20 border-t-electric animate-spin shadow-[0_0_30px_rgba(180,255,0,0.2)]" style={{ animationDuration: '2s' }} />
        ...
        <div className="flex flex-col items-center">
          <div className="font-mono text-[10px] md:text-xs text-white/60 tracking-[0.3em] uppercase flex items-center shadow-black drop-shadow-md">
            ...
            <span className="text-electric font-bold w-12 text-right tracking-widest">{progress}%</span>
          </div>
        </div>
      </div>
    </div>
